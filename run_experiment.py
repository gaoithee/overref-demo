...
def main(args):
    ...
    if args.load_results:
        ...
        _compute_and_save_metrics(raw_df)
        return
    ...
    selected_checkpoints = {
        k: v for k, v in OLMO_CHECKPOINTS.items()
        if k in args.checkpoints
    } if args.checkpoints else OLMO_CHECKPOINTS

    selected_prompts = {
        k: v for k, v in SYSTEM_PROMPTS.items()
        if k in args.system_prompts
    } if args.system_prompts else SYSTEM_PROMPTS
    ...
    for model in iter_checkpoints(selected_checkpoints, selected_prompts, GENERATION):
        logger.info("Running inference for: %s", model.checkpoint_name)
        responses = run_generation(model, prompts, GENERATION.batch_size)
        ...
        # Unload model and free GPU memory before loading the next one
        model.unload()
...
def _compute_and_save_metrics(raw_df: pd.DataFrame) -> None:
